...
export async function middleware(request: NextRequest) {
  ...
  const protectedPath = pathname.startsWith("/api") || ["/dashboard", "/participants", "/attendance", "/meals", "/teams", "/outreach", "/digital-evangelism", "/attendance-meal-reports", "/certificates", "/settings"].some((route) => pathname === route || pathname.startsWith(`${route}/`));
...
export const config = {
  matcher: ["/dashboard/:path*", "/participants/:path*", "/attendance/:path*", "/meals/:path*", "/teams/:path*", "/outreach/:path*", "/digital-evangelism/:path*", "/attendance-meal-reports/:path*", "/certificates/:path*", "/settings/:path*", "/api/:path*"]
};
